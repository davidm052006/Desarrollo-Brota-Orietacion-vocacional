...
| `admin/preguntasController.js` no crea preguntas de opciones que puntúen de verdad | Pendiente técnico, confirmado agosto 2026 | Las preguntas creadas desde el panel admin siguen escribiendo la columna legada `preguntas.opciones` (JSONB), no las tablas relacionales `opciones`/`pesos_opciones`. Los tipos abiertos agregados sí funcionan porque no necesitan puntuación. `institucion/preguntasController.js` gestiona las tablas relacionales y es el flujo recomendado para cuestionarios propios de una institución. |
