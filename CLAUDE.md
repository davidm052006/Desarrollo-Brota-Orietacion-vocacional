...
- **`pages/dashboard/components/BannerCarousel.jsx`** (agosto 2026) — carrusel de la home del Dashboard, 5 slides: la primera es el estado real del test vocacional (mismo estado que tenía el viejo `ContinueSection.jsx`, que ya no se usa ahí — el archivo sigue existiendo por si se reutiliza en otro lado), las otras 4 son banners informativos con copy inventado (SNIES, RIASEC, Rutas, Comunidad) — **contenido de relleno, no datos reales**, ajustar si se quiere promocionar otra cosa. Autoplay cada 6s (`setInterval`, se pausa en `onMouseEnter`), navegación manual con flechas y dots que reinician el ciclo.
...
**⚠️ Requiere correr `backend/scripts/migration_broti.sql` en Supabase antes de probar** (ya incluida en `backend/setup_database.sql` para instalaciones nuevas) — agrega `perfiles_usuario.broti_config JSONB DEFAULT '{}'`.

- **`/dashboard/broti`** (`Broti.jsx`, nuevo) — dos pestañas: "Mi Broti" (preview + quitar lo equipado) y "Tienda". Se llega desde un botón en `/dashboard/racha` y desde `Perfil.jsx`. Por ahora la única categoría es **`fondo`** (6 assets reales en `frontend/public/broti/fondos/` — cielo con arcoíris, bosque encantado, arrecife, desierto, ciudad de noche, espacio; 2 gratis + 4 premium), se dibuja como `background: url(...) center / cover` detrás de la mascota, no como badge encima.
- **Se probó y se descartó** un sistema de lentes/accesorio fusionados de verdad dentro del SVG del mascota (piezas sueltas combinables con `fondo`) — el resultado nunca terminó de verse bien (choque de estilo entre el mascota base y los accesorios, y problemas de z-order con las orejas tapando las patillas de los lentes). Se sacó todo (`BrotiMascota.jsx`, `brotiFusion.js`, los assets en `frontend/public/broti/lentes|accesorios/`) — quedan de referencia en `logos/variantes/` (lámina de stickers generada, recortes, SVGs de Recraft) por si se retoma esa idea más adelante, pero no están en uso.
- **Plan actual**: en vez de piezas combinables, van a ser **"variantes" de Broti ya armadas como imagen completa** (el usuario las prepara aparte con IA y las pasa listas) — el usuario elige una variante entera en vez de combinar lentes+accesorio+fondo por separado. Cuando lleguen esos assets, se agrega una categoría `variante` a `frontend/src/utils/brotiCatalog.js` con el mismo patrón que `fondo` (mismo `getItem`/`getItemsPorCategoria`), y `BrotiAvatar.jsx`/`Broti.jsx` pasan a elegir el `src` de la imagen según `config.variante` en vez del path fijo a `logo-feliz.svg` — `fondo` se mantiene como capa aparte, sin cambios.
