...
| `/dashboard/institucion` | `pages/dashboard/institucion/PerfilInstitucion.jsx` | **NUEVO agosto 2026** — solo accesible con rol `institucion`. Datos oficiales de la institución (solo lectura) + cuestionario propio editable (contacto, teléfono, descripción). Ver sección "Rol institución" más abajo |
| `/dashboard/institucion/programas` | `pages/dashboard/institucion/MisProgramas.jsx` | **NUEVO agosto 2026** — lista los programas de la institución vinculada, edición inline de descripción/requisitos/costo/activo |
| `/dashboard/institucion/usuarios` | `pages/dashboard/institucion/Usuarios.jsx` | **NUEVO agosto 2026** — crear/cargar (CSV/Excel) estudiantes propios, vinculados automáticamente a la institución |
| `/dashboard/institucion/cuestionarios` | `pages/dashboard/institucion/Cuestionarios.jsx` | **NUEVO agosto 2026** — CRUD de cuestionarios propios + editor de preguntas con opciones y pesos por categoría reales |
...
| `institucion.js` | `/api/institucion` | **NUEVO agosto 2026**, protegida por `verificarInstitucion` — `GET/PATCH /programas[/:id]`, `GET/POST /usuarios` + `POST /usuarios/masivo` + `DELETE /usuarios/:id`, `GET/POST/PATCH/DELETE /cuestionarios[/:id]`, `GET/POST/PATCH/DELETE /preguntas[/:id]` — todo scoped a la institución vinculada a la cuenta (ver sección "Rol institución") |
...
| `admin/preguntasController.js` no crea preguntas que puntúen de verdad | Bug real, confirmado agosto 2026 | `createPregunta`/`updatePregunta` solo escriben la columna legada `preguntas.opciones` (JSONB) — nunca tocan las tablas relacionales `opciones`/`pesos_opciones` que `perfilController.obtenerCuestionario` lee de verdad para armar el test y calcular el perfil vocacional. Una pregunta creada desde el panel admin hoy no suma puntos a ninguna categoría. `institucion/preguntasController.js` (ver sección "Rol institucion") sí lo hace bien — sería el modelo a seguir si se arregla el del admin. |
...
## Rol "institucion" (agosto 2026)

Cuentas que representan una institución educativa del catálogo (`instituciones`), con acceso acotado a sus propios datos — a diferencia de admin/moderador, no tocan nada fuera de lo suyo.

- **Alta**: solo desde el panel admin (`ModalNuevoUsuario.jsx`, buscador con debounce contra `GET /api/admin/instituciones` en vez de un `<select>` con miles de filas) — decisión consciente, no self-signup público, para que nadie pueda registrarse suplantando una institución real. El admin vincula la cuenta a una fila de `instituciones` (`institucion_id`); "nombre"/"apellido" en ese alta son el contacto administrativo inicial, no el nombre de la institución.
  El botón "🏫 ¿Sos una institución?" en `LoginCard.jsx` NO crea la cuenta — manda a `/contacto?asunto=institucion` (reutiliza el flujo de `/api/contacto` con el asunto "Alianzas institucionales" ya existente). Desde `ContactosSection.jsx` (panel admin), el botón "🔑 Dar credenciales" en `ContactoCard.jsx` navega a Usuarios con el `ModalNuevoUsuario` pre-lleno (`AdminPanel.jsx` levanta el estado `prefillUsuario`, mismo patrón que `filtroCuestionarioId` para "Ver preguntas").
- **`perfiles_usuario.institucion_id`** — FK a `instituciones(id)` con `ON DELETE SET NULL`, agregada como columna plana + `ALTER TABLE ... ADD CONSTRAINT` **después** de `CREATE TABLE instituciones` en `setup_database.sql` (esta última se define más abajo en el script; una FK inline en el `CREATE TABLE` de `perfiles_usuario` habría fallado). Migración para bases existentes: `backend/scripts/historico/migration_rol_institucion.sql`.
  **⚠️ Trade-off aceptado, no perseguir como bug**: la sync MEN borra y recrea `instituciones` completa en cada corrida (`sincronizacionController.js`) con UUIDs nuevos → toda cuenta institución queda desvinculada tras una sync. Se re-vincula a mano desde "Editar usuario" en el panel (`PATCH /api/admin/usuarios/:id` ahora acepta `institucion_id`). Mismo tipo de trade-off que ya existía para `recomendaciones` (bug #3 de la tabla de arriba).
- **`institucion_contacto`/`institucion_descripcion`** (+ reutiliza la columna `telefono` ya existente) — el "cuestionario" propio que la cuenta completa en `/dashboard/institucion` tras el alta (`PerfilInstitucion.jsx`), vía el mismo `PATCH /api/perfil/:userId` que ya usa `Perfil.jsx` (no hay endpoint aparte — es la misma fila de `perfiles_usuario`). `institucion_id` NO se acepta en ese PATCH a propósito: solo un admin re-vincula.
- **`GET /api/perfil/:userId`** ahora embebe `institucion:instituciones(...)` cuando hay `institucion_id` (PostgREST resuelve el embed por la FK). Ojo si se vuelve a tocar `actualizarRacha()` en `perfilController.js`: su propio `.update().select()` no pide ese embed, así que el merge tiene que hacer `{ ...perfil, ...actualizado }` (perfil primero) o el campo `institucion` desaparece en la primera carga del día.
- **`backend/src/middlewares/verificarInstitucion.js`** + **`backend/src/routes/institucion.js`** (`/api/institucion/*`) — self-contained como los demás middlewares de rol, adjunta `req.institucionId` (nunca confiar en uno que mande el body/query). `controllers/institucion/programasController.js`: `GET /programas` (solo los de su institución) y `PATCH /programas/:id` (solo `descripcion`/`requisitos`/`costo_matricula`/`activo` — nombre/tipo/area_academica son dato oficial del MEN, no editable acá), valida pertenencia antes de tocar cualquier fila.
- **`backend/src/utils/crearUsuario.js`** (agosto 2026) — `crearUsuarioUnico()` se extrajo de `admin/usuariosController.js` a un util compartido, porque ahora también lo usa `institucion/usuariosController.js` (una institución crea/carga masiva sus propios estudiantes: `POST /api/institucion/usuarios[/masivo]`, `DELETE /:id`). El rol y el `institucion_id` se fuerzan server-side en el controller de institución (siempre `'estudiante'` + la propia institución) — nunca lo que mande el body, para que una institución no pueda crearse un admin o asignarse a otra institución.
- **Cuestionarios propios por institución** — `cuestionarios.institucion_id` (nullable, `NULL` = el global de siempre) vía `controllers/institucion/cuestionariosController.js` (`GET/POST/PATCH/DELETE /api/institucion/cuestionarios[/:id]`, siempre scoped a `req.institucionId`; activar uno solo desactiva los OTROS de la MISMA institución, nunca el global ni los de otra). `perfilController.obtenerCuestionario` resuelve primero el cuestionario activo de la institución del estudiante (si tiene una) y cae al global si no hay uno — un cuestionario de institución nunca reemplaza el global para el resto de la plataforma.
  **⚠️ A diferencia del CRUD de Preguntas que ya tenía el admin** (`admin/preguntasController.js` — solo escribe la columna legada `preguntas.opciones` JSONB, que nadie lee para calcular resultados), **`institucion/preguntasController.js` sí gestiona las tablas relacionales reales** `opciones`/`pesos_opciones` que `obtenerCuestionario` usa de verdad para armar el test y puntuar — ver la fila de deuda técnica de más abajo, el CRUD del admin sigue roto en ese sentido y quedó fuera de este cambio.
  Migración pendiente: `backend/scripts/historico/migration_cuestionarios_institucion.sql` (agrega `cuestionarios.institucion_id`) — **mientras no se corra**, `obtenerCuestionario` cae con gracia al comportamiento de siempre (chequea el código de error `42703`, columna inexistente) en vez de romper el test vocacional para todos.
- **Frontend**: `hooks/useInstitucion.js` (mismo patrón que `useModeracion.js`) da `esInstitucion` + `necesitaCuestionario`. Panel de 4 secciones bajo `pages/dashboard/institucion/` (`PerfilInstitucion`, `MisProgramas`, `Usuarios`, `Cuestionarios`), compartiendo `components/InstitucionNav.jsx` como sub-nav. `TopNavbar.jsx` agrega el tab "Mi institución" condicional, sin sacar ningún tab existente (una institución todavía ve Test vocacional/Rutas aunque no le apliquen — pospuesto a propósito, no ocultar tabs sin que el usuario lo pida). `utils/vocacionalCategorias.js` ahora exporta también `CATEGORIA_OPCIONES` (antes solo `getCategoriaInfo`/`storageKey`), reutilizado por el editor de pesos de `Cuestionarios.jsx` — no es una 5ta lista nueva de categorías.
