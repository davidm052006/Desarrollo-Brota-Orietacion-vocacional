...
| `/dashboard/rutas` | `pages/dashboard/Rutas.jsx` | chips de área académica (mismas 14 claves que `area_academica`) → contenido estático de `contenido_rutas` (temas previos, proyectos, links de búsqueda de YouTube). **Sin IA en tiempo real** — el contenido se escribió una sola vez (agosto 2026, ver `backend/scripts/migration_rutas.sql`) para no generar costo ni dependencia externa |
